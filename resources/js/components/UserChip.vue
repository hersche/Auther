            ...
            <v-card>
                ...
                <v-card-text :style="'background-image:url('+item.background+'); background-position: center; background-size: cover;'"
                    ...
                    <v-chip class="text-center" color="blue" disabled text-color="white">
                        <v-subheader dark>{{ item.name }} ({{ item.username }})</v-subheader>
                    </v-chip>
                </v-card-text>
                ...
                <v-card-text>
                    <div v-if="loginId!=item.id">
                        <div v-if="friendstatus==0">No friend-thing yet
                            <v-btn @click="changeFriend('/internal-api/friends/friendRequest', item.id)">Send
                                friendrequest
                            </v-btn>
                        ...
                        <div v-if="friendstatus==3">Denied friend
                            <v-btn @click="changeFriend('/internal-api/friends/acceptRequest', item.id)">Accept
                                request
                            </v-btn>
                        </div>
                        <div v-if="friendstatus==4">Blocked friend
                            <v-btn @click="changeFriend('/internal-api/friends/unblock', item.id)">Unblock</v-btn>
                        </div>
